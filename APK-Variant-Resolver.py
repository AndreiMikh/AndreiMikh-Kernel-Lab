...
def detectregionalvariant():
    ...
    # Build the manifest information string exactly like the Bash version.
    infomanifest = (
    ...
    # ColorOS / China indicators.
    if contains(r"coloros|cn|china", infomanifest):
        scorecolor += 6

    # OxygenOS / Global indicators.
    if contains(r"oxygen|oneplus|global|eu|india", infomanifest):
        scoreoxygen += 6

    # OPlus is a shared signal and contributes to both ROM scores.
    if contains(r"oplus", infomanifest):
        scorecolor += 1
        scoreoxygen += 1

    # Analyze the kernel image when available.
    if IMAGEPATH.is_file():
        ...
        # Show the same relevant debug strings as the original workflow.
        ...
        # Strong ColorOS signal.
        if contains(r"coloros", lowerstrings):
            scorecolor += 4

        # China region hardware configuration.
        if contains(
        ...
        # Strong OxygenOS signal.
        if contains(r"oxygen", lowerstrings):
            scoreoxygen += 4

        # Global region hardware configuration.
        if contains(
        ...
        # Additional weaker global hints.
        if contains(
        ...
        # OnePlus is a weak OxygenOS signal.
        if contains(r"oneplus", lowerstrings):
            scoreoxygen += 1

        # OPlus is shared by both ecosystems.
        if contains(r"oplus", lowerstrings):
            scorecolor += 1
            scoreoxygen += 1

    else:
        # If the image does not exist, use manifest information only.
        print("⚠️ Kernel Image Not Found")
        print("⚠️ Using Manifest-Only Detection")
        print()

    # Preserve the original final decision logic.
    ...
    # Preserve the original confidence thresholds.
    if maxscore >= 10:
        confidence = "High"
    ...
    # Export results for later workflow steps.
...
def scoreartifacts(artifacts):
    ...
    # Skip runs without relevant APK/manager/release/build artifacts.
    if not contains(
...
def resolveapk():
    ...
    # Preserve the original output directory naming.
    managerapkdir = (
        WORKSPACE
    ...
    # Preserve first-highest-score selection behavior.
    for runid in runs:
        print("------------------------------")
    ...
    # Verify the selected run before downloading.
    ...
    # Download and extract each Manager artifact.
    for artifact in managerartifacts:
        url = artifact.get(
            "archive_download_url"
        )
    ...
    # Remove every extracted file except APKs.
    print(
    ...
    # Preserve the original ReSukiSU-specific filtering.
...
# ---------------------------------------------------------------------------
# Manager APK validation and collection
# ---------------------------------------------------------------------------
...
def validateandcollect(managerapkdir):
    ...
    # Find all APKs produced by the resolver.
    apkfiles = [
        path
        for path in managerapkdir.rglob(
            "*.apk"
    ...
    if apkcount > 0:
        ...
        # Export status for later workflow steps.
        appendgithubenv(
            "APKMANAGER",
            apkmanager,
        )
        ...
        # Verify the flattened APK count.
        flatfiles = [
            path
        ...
        # Preserve the original no-APK behavior.
        apkmanager = "false"
        apkcount = 0
...
def main():
    ...
    # Stage 1: Regional ROM detection.
    detectregionalvariant()

    # Stage 2: Manager APK workflow/artifact resolution.
    managerapkdir = resolveapk()

    # Stage 3: Validate and flatten Manager APKs.
