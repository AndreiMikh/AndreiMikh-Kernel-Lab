...
info()  { echo "[Info] $*"; }
pass()  { echo "[Passed] $*"; ((PATCHED++)) || true; }
skip()  { echo "[Skipped] $*"; ((SKIPPED++)) || true; }
fail()  { echo "[Failed] $*"; ((FAILED++)) || true; }
...
# Validate Thin Wrapper Header
if [ ! -f "${SCRIPTDIR}/include/linux/lz4.h" ]; then
  fail "LZ4 Wrapper Header Not Found: ${SCRIPTDIR}/include/linux/lz4.h"
  exit 1
fi
...
# Modify Crypto/LZ4C and Crypto/LZ4HCC to Add the Arm64 NEON Branch
echo ""
...
if [ ! -f "$INCFS" ]; then
  ...
  if already_has 'system_power_efficient_wq' "$INCFS"; then
    ...
    if already_has 'system_power_efficient_wq' "$INCFS"; then
      pass "DataMgmtC Queue Delayed Work Replacement Completed"
    else
      fail "DataMgmtC Failed to Replace Queue Delayed Work"
    fi
  fi
fi
